...
import { TranslatedResumeProps, TranslatedSectionProps } from "../../../types/resume/"
import { Contacts, DownloadPDF, Header, Section, Summary } from "../components/"
import { formatDate, showSection, translateDates, translateResume, translateSection } from "../types.ts"
...
export const Main = async ({ lng }: ElementProps) => {
    ...
    const formatDate: formatDate = (date) => {
        if (date === undefined) {
            ...
            const capitalizedDateStr = dateStr[0].toLocaleUpperCase(lng) + dateStr.substring(1)
    ...
    const translateSection: translateSection = (section) => {
        ...
        for (const item of section.items) {
            ...
            if (section.subitem) translated.subitems?.push(t(`sections.${section.name}.items.${item}.subitem`))
            if (hasDates) {
    ...
    return (
        <MainWrapper>
            ...
            <ResumeWrapper>
                ...
                {translated.sections.map((section, index) => showSection(section, index))}
            </ResumeWrapper>
        </MainWrapper>
    )
}
