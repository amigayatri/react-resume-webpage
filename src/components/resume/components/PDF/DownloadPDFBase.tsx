import { Document, Font, Image, Link, Page, PDFDownloadLink, Styles, StyleSheet, Text, View } from "@react-pdf/renderer"
...
import { PDFDownload, PDFDownloadText, PDFDownloadWrapper } from "./DownloadPDF.styled.ts"
...
const PDF = ({ resume, isDark }: PDFProps) => {
    const styles = isDark ? StyleSheet.create(darkStyle) : StyleSheet.create(lightStyle)
            ...
            <View key={`section-n-${idx}`} style={styles.section}>
                ...
                {section.items.map((item, idx) => (
                    <View style={styles.item} key={item}>
                        <Text>
                            {item}
                            {section.dates !== undefined && <Text style={styles.date}>{section.dates[idx]}</Text>}
                        </Text>
...
export const DownloadPDFBase = ({ resume, lng, t, isDark }: DownloadPDFBaseProps) => {
    const fileName = t("download.title", { fullName: "Amira Gayatri" }).split(" ").join("_")
    return (
        <PDFDownloadWrapper>
            <PDFDownloadLink document={<PDF resume={resume} isDark={isDark} />} fileName={fileName}>
                <PDFDownload>
