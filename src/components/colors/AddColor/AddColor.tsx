...
export const AddColor = (props: { add: (arg0: string) => void; regenerate: () => void }) => {
	...
	return (
		<Wrapper>
			...
			<InputWrapper>
				{showColorSelector && <ColorSelector onChange={(e)=>props.add(e.target.value)} defaultValue="#666666" type="color" />}
				{showColorTextInput && <ColorTextInput onChange={(e) => handleColorList(e.target.value)} ></ColorTextInput>}
				{showChangeSteps && (
					<ChangeSteps onChange={(e)=>handleSteps(e.target.value)} min={2} max={32} defaultValue={variationSteps} type="number" step={1} />
				)}
			</InputWrapper>
		</Wrapper>
	)
