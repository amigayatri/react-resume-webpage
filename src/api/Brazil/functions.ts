...
import { HolidayProps, RawHoliday, RawMunicipal } from "../../types/holidays"

type readJsonState = (state: string) => Promise<RawMunicipal[]>
export const readJsonState: readJsonState = async (state) => {
    const response = await fetch("/brazilian-holidays/" + state.toLowerCase() + ".json")
...
type isMunicipal = (holiday: RawMunicipal | RawHoliday) => holiday is RawMunicipal
...
export const hasPassed: hasPassed = (now, holiday) => {
    ...
    if (holidayYear > currYear || holidayMonth > currMonth || (holidayMonth === currMonth && holidayDate >= currDay)) {
        return false
    }
    return true
}
...
export const formatHoliday: formatHoliday = (raw, holiday, customName, isNextYear, now) => {
    if (isMunicipal(raw)) {
